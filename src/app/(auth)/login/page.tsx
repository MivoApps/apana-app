...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Limpiar caché vieja de Zustand de sesiones anteriores
      localStorage.clear();
      ...
      let errorMsg = 'Esta cuenta no existe o los datos ingresados son incorrectos.';
      if (err?.code === 'auth/network-request-failed') {
        errorMsg = 'Error de red. Verifica la conexión Wi-Fi de tu celular.';
      ...
      <main className="w-full max-w-sm mx-auto flex flex-col items-center gap-6 relative z-10">
        ...
        <div className="w-full bg-white rounded-2xl p-6 border border-[#bccac0]/40 shadow-xs flex flex-col gap-6">
          ...
          <div className="text-center flex flex-col items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-[#0b1c30] tracking-tight">Bienvenido</h1>
            <p className="text-sm text-[#3d4a42]">Ingresa tus datos para gestionar tu tienda.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex justify-end w-full">
              <Link href="/forgot-password" className="text-sm font-medium text-[#059669] hover:underline">
        ...
        {/* Footer Register Prompt & Terms */}
        <div className="flex flex-col items-center gap-3 text-center">
          <p className="text-sm text-[#0b1c30]">
            ¿No tienes cuenta?{' '}
            <Link href="/register" className="font-semibold text-[#059669] hover:underline">
              Regístrate gratis
            </Link>
          </p>

          <p className="text-[11px] text-slate-400 max-w-xs leading-tight">
            Al acceder, confirmas que aceptas nuestros{' '}
            <button
              type="button"
              onClick={() => setIsTermsOpen(true)}
              className="text-[#059669] underline font-medium hover:text-[#006c49]"
            >
              Términos de Servicio y Declaración Jurada
            </button>
            .
          </p>
        </div>
